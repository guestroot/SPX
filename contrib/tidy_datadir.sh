...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete SPX database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no SPX datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old SPX datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected SPX 0.7 datadir."
    ;;
  3)
    echo "Detected SPX pre-0.8 datadir."
    ;;
  4)
    echo "Detected SPX 0.8 datadir."
    ;;
esac
