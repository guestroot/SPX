...
*Setup instructions for a Gitian build of SPX Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the SPX
...
- [Building SPX Core](#building-SPX-core)
...
Clone the git repositories for SPX and Gitian.
...
in the SPX repository under 'Fetch and create inputs' to install sources which require
...
To build SPX Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the SPX repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/SPX/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/SPX-project/SPX
...
For example:
```bash
URL=https://github.com/thrasher-/SPX.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit SPX=${COMMIT} --url SPX=${URL} ../SPX/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit SPX=${COMMIT} --url SPX=${URL} ../SPX/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit SPX=${COMMIT} --url SPX=${URL} ../SPX/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the SPX git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../SPX/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/SPX
SIGPATH=/some/root/path/SPX-detached-sigs

./bin/gbuild --url SPX=${BTCPATH},signature=${SIGPATH} ../SPX/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/SPX-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/SPX-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/SPX-osx-build.assert
```
...
[SPX-project/gitian.sigs.SPX](https://github.com/SPX-project/gitian.sigs.SPX/) repository, or if that's not possible create a pull
