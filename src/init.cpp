...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (showDebug)
    {
        ...
        strUsage += HelpMessageOpt("-checkblockindex", strprintf("Do a full consistency check for mapBlockIndex, setBlockIndexCandidates, chainActive and mapBlocksUnlinked occasionally. Also sets -checkmempool (default: %u)", Params(CBaseChainParams::MAIN).DefauSPXonsistencyChecks()));
        strUsage += HelpMessageOpt("-checkmempool=<n>", strprintf("Run checks every <n> transactions (default: %u)", Params(CBaseChainParams::MAIN).DefauSPXonsistencyChecks()));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/SPX-project/SPX>";
    const std::string URL_WEBSITE = "<https://SPX.org>";
...
bool AppInitParameterInteraction()
{
    ...
    int ratio = std::min<int>(std::max<int>(GetArg("-checkmempool", chainparams.DefauSPXonsistencyChecks() ? 1 : 0), 0), 1000000);
    if (ratio != 0) {
        mempool.setSanityCheck(1.0 / ratio);
    }
    fCheckBlockIndex = GetBoolArg("-checkblockindex", chainparams.DefauSPXonsistencyChecks());
